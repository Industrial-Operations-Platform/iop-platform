...
Accepted on 2026-09-14 under [IOP-003](../../planning/items/IOP-003-api-contract-strategy.md).
The owner explicitly accepted the proposed strategy. This record establishes the
API design baseline; endpoint implementation remains separately scoped.
...
| Versioning option | Tradeoff | Decision |
| --- | --- | --- |
...
Use OpenAPI as the language-neutral HTTP contract artifact. Use a Nest
...
| Authoring approach | Tradeoff | Decision |
| --- | --- | --- |
...
defines operations, schemas, responses and security descriptions. This decision
...
metadata. For potentially large detail lists, use opaque cursor pagination
...
Use `traceId` as a correlation extension and `errors` for validation entries
...
This decision does not select scope transport, an identity provider, a session
...
Acceptance is reflected in ARCHITECTURE.md, module contract guidance and the
completed IOP-003 design item. IOP-016 owns API bootstrap; IOP-022 owns error-model
...
| Scenario | Expected behavior under the decision | Design review |
| --- | --- | --- |
